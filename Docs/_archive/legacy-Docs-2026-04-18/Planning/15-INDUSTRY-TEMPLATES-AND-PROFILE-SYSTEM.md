# Industry Templates & Business Profile System

## Overview

This document defines the complete implementation for industry-specific templates that provide accurate simulations from day 1, combined with progressive learning through pattern extraction.

**Core Principle:** New users get realistic simulations immediately using industry templates. The system then learns from their actual conversations and refines the profile over time.
...
├─ System creates Tenant with selected industry
└─ System creates BusinessProfile seeded with industry template
   ↓
First Simulation (Day 1)
├─ Uses template data (serviceDescription, targetClient, etc.)
├─ AI persona is industry-specific and realistic
└─ Business owner practices with accurate scenarios
   ↓
After Simulation Completes
├─ Pattern extraction analyzes conversation
├─ Updates BusinessProfile with extracted patterns
└─ Increases completion percentage
   ↓
Future Simulations
├─ Uses BOTH template data AND extracted patterns
├─ Gets progressively more personalized
└─ System learns user's specific approach
   ↓
Business Profile Page
├─ Basic Info tab: User can edit template data
├─ Extracted Patterns tab: Auto-generated (read-only)
└─ Shows combination of both
```
...
model BusinessProfile {
  ...
  //   style: "data-driven" | "emotional" | "educational",
  ...
  //   minBudget: number,
  //   maxBudget: number,
  //   flexibilityFactors: ["timeline", "scope", "payment_terms"],
  //   dealBreakers: ["budget_below_X", "unrealistic_timeline"]
  // }
  
  qualificationCriteria Json?     @db.JsonB
  // Structure: {
  //   mustHaves: ["minimum_credit_score", "proof_of_income"],
  //   dealBreakers: ["credit_below_620", "no_down_payment"],
  //   greenFlags: ["pre_approved", "serious_timeline"],
  //   redFlags: ["just_browsing", "unrealistic_expectations"]
  // }
  
  objectionHandling     Json?     @db.JsonB
  // Structure: {
  //   priceObjection: "Explains value, offers payment plans",
  //   timelineObjection: "Sets realistic expectations",
  //   competitorObjection: "Focuses on unique value"
  // }
  
  decisionMakingPatterns Json?    @db.JsonB
  // Structure: {
  //   whenToSayYes: ["qualified_lead", "realistic_budget"],
  //   whenToSayNo: ["not_ready", "wrong_fit"],
  //   warningSignsToWatch: ["hesitation", "budget_mismatch"]
  // }
  
  knowledgeBase         Json?     @db.JsonB
  // Structure: {
  //   expertiseAreas: ["FHA_loans", "VA_loans", "refinancing"],
  //   commonAnswers: [{ question: "...", answer: "..." }]
  // }
  ...
  // 20% = Has basic template
  // 40% = 1 simulation completed
  // 60% = 3 simulations completed
  // 80% = 5 simulations completed
  // 100% = 10+ simulations completed
...
### **Tenant Model (No Changes Needed):**

```prisma
model Tenant {
  id                String   @id @default(uuid())
  businessName      String
  industry          String   // ← Already exists, used for template selection
  // ... existing fields ...
}
```
...
```typescript
// lib/templates/industry-templates.ts
...
export interface IndustryTemplate {
  ...
  // Template data (seeds BusinessProfile)
  serviceDescription: string;
  targetClientType: string;
  typicalBudgetRange: string;
  commonClientQuestions: string[];
  
  // Scenario-specific personas (for AI simulations)
  scenarios: {
...
## Industry Templates (Complete Library)
...
```typescript
{
  ...
  serviceDescription: "I help clients secure home loans with competitive rates and guide them through the mortgage process from pre-approval to closing.",
  
  targetClientType: "First-time homebuyers, families looking to refinance, real estate investors",
  ...
  scenarios: {
    PRICE_SENSITIVE: {
      ...
      openingLine: "Hi, I'm looking to buy my first home around $280k. I need to make sure I'm getting the best rate possible because every percentage point matters to my monthly payment. What kind of rates can you offer?",
      typicalObjections: [
    ...
    INDECISIVE: {
      ...
      openingLine: "Hi, my partner and I are thinking about buying a home around $350k. We have about $50k saved for a down payment, but we're not sure if we should wait or if rates might go down. What do you think?",
      typicalObjections: [
    ...
    DEMANDING: {
      ...
      openingLine: "I'm looking at a $600k investment property and I need this closed in 30 days. I have excellent credit (780+) and 25% down. I expect rates in the low 6% range. Can you deliver on that timeline, or should I look elsewhere?",
      typicalObjections: [
    ...
    TIME_PRESSURED: {
      ...
      openingLine: "Hi, I found my dream home at $320k but the seller wants to close in 3 weeks or they'll move to another buyer. My credit is good (710) and I have 15% down. Can you help me get approved and close that fast?",
      typicalObjections: [
    ...
    HIGH_BUDGET: {
      ...
      openingLine: "I'm purchasing a $1.2 million property in [upscale area] and I'm looking for a mortgage advisor who specializes in jumbo loans. I have significant assets and want to explore the best financing structure for tax optimization. What's your experience with high-net-worth clients?",
      typicalObjections: [
...
```typescript
{
  ...
  serviceDescription: "I create beautiful, functional spaces that reflect your personality and lifestyle, from concept to completion.",
  
  targetClientType: "Homeowners renovating, new construction clients, commercial space owners",
  ...
  scenarios: {
    PRICE_SENSITIVE: {
      ...
      personality: "Budget-conscious, has Pinterest inspiration, DIY willing",
      openingLine: "Hi! My partner and I just bought our first home and we want to renovate the living room and dining area. We have about $15k total. We love modern farmhouse style but we're worried that's too expensive. Can you work with our budget and still make it look amazing?",
      typicalObjections: [
    ...
    INDECISIVE: {
      ...
      personality: "Overthinking, seeks validation, changes mind frequently",
      openingLine: "I'm redoing my master bedroom and I have about $25k to spend. I love the coastal vibe but also really like modern minimalist, and some industrial elements. I have so many ideas but I don't know how to make them work together. Where do I even start?",
      typicalObjections: [
    ...
    DEMANDING: {
      ...
      openingLine: "I'm looking for a designer to completely redo my 4,000 sq ft home. I have a very specific vision - think Restoration Hardware meets Kelly Wearstler. Budget isn't the issue, but I need perfection. I've worked with three designers before and wasn't satisfied. Can you execute at this level?",
      typicalObjections: [
    ...
    TIME_PRESSURED: {
      ...
      openingLine: "Hi, I'm relocating for work and I need my new home designed and ready in 6 weeks. I have $30k to work with for the main living areas. I know that's tight, but I'm flexible on choices if it means getting done on time. Can you make this happen?",
      typicalObjections: [
    ...
    HIGH_BUDGET: {
      ...
      personality: "Sophisticated, expects white-glove service, relationship-focused",
      openingLine: "I'm designing a vacation property in [luxury location] and I need a designer who can handle the entire process - architecture coordination, custom furniture, art curation, everything. Budget is $150k+. I'm looking for someone who understands high-end clients and can manage this end-to-end. Is this your specialty?",
      typicalObjections: [
...
```typescript
{
  ...
  serviceDescription: "I help businesses improve operations, develop growth strategies, and solve complex business challenges through data-driven insights and proven frameworks.",
  ...
  scenarios: {
    ...
    INDECISIVE: {
      ...
      openingLine: "I run a $2M/year manufacturing business and I'm struggling with operations, hiring, and scaling. I've read books and taken courses but I'm still confused about what to prioritize. I have about $15k budget. I've never worked with a consultant - how does this work? Where do we even start?",
      typicalObjections: [
    ...
    DEMANDING: {
      ...
      openingLine: "I'm the CEO of a $50M B2B company and we're considering expansion into a new vertical. I've worked with McKinsey on past initiatives. I need strategic analysis and market entry recommendations at that level of rigor. Budget is $50k+ for the right advisor. What's your experience with market entry strategy for companies at our scale?",
      typicalObjections: [
    ...
    TIME_PRESSURED: {
      ...
      openingLine: "I have an acquisition opportunity that I need to evaluate in the next 3 weeks. The target company does $5M revenue. I need due diligence, financial analysis, and integration planning fast. Budget is $20k. Can you turn this around in that timeframe with quality work?",
      typicalObjections: [
    ...
    HIGH_BUDGET: {
      ...
      openingLine: "We're a $200M company going through digital transformation and organizational restructuring. This is a 6-12 month initiative with budget of $100k+. We need someone who can work alongside our leadership team, not just deliver a report. What's your experience leading transformations at this scale?",
      typicalObjections: [
...
```typescript
{
  ...
  serviceDescription: "I help buyers and sellers navigate the real estate market with expert guidance, market insights, and personalized service to achieve their property goals.",
  
  targetClientType: "First-time buyers, sellers, investors, luxury clients",
  ...
  scenarios: {
    PRICE_SENSITIVE: {
      ...
      openingLine: "Hi, I'm looking to buy my first home with a budget of $350k max. The market seems crazy right now and I'm worried about overpaying or missing out. I've heard horror stories about bidding wars. How do you help buyers get a fair deal in this market?",
      typicalObjections: [
    ...
    INDECISIVE: {
      ...
      openingLine: "My partner and I are looking to buy but we can't decide between a newer home in the suburbs or an older home closer to downtown. We have $500k budget. We've been looking for 6 months and keep going back and forth. How do you help clients figure out what they really want?",
      typicalObjections: [
    ...
    DEMANDING: {
      ...
      personality: "High standards, impatient, expects constant updates",
      openingLine: "I'm selling my $2.5M property and I need an agent who specializes in luxury markets. I expect professional photography, staging, targeted marketing to high-net-worth buyers, and regular updates. I've interviewed three agents already. What makes you different in the luxury segment?",
      typicalObjections: [
    ...
    TIME_PRESSURED: {
      ...
      openingLine: "I'm relocating for work in 60 days and I need to sell my $600k home fast. I'm willing to price it competitively to sell quickly but I don't want to give it away. What's your strategy for quick sales and can you handle the timeline pressure?",
      typicalObjections: [
    ...
    HIGH_BUDGET: {
      ...
      openingLine: "I'm looking at luxury properties in the $3-5M range, either as a primary residence or investment property. I'm interested in exclusive listings and off-market opportunities. What's your experience working with high-net-worth clients and what kind of access do you have to luxury inventory?",
      typicalObjections: [
...
```typescript
{
  ...
  serviceDescription: "I help clients build and protect wealth through personalized financial planning, investment strategies, and comprehensive financial guidance.",
  
  targetClientType: "Young professionals, families, pre-retirees, high-net-worth individuals",
  ...
  scenarios: {
    PRICE_SENSITIVE: {
      ...
      openingLine: "Hi, I'm 28 and I've saved about $50k that's just sitting in a savings account. I know I should invest it but I'm intimidated by the options and worried about fees. I've looked at robo-advisors but I'd like human guidance. How do you charge and is it worth it for someone at my level?",
      typicalObjections: [
    ...
    INDECISIVE: {
      ...
      openingLine: "I'm 45 with about $300k saved between my 401k and savings. I keep reading conflicting advice about what to do - some say be aggressive, others say protect what I have. I'm worried I'm not on track for retirement but also scared of losing money. How do you help someone figure out the right strategy?",
      typicalObjections: [
    ...
    DEMANDING: {
      ...
      openingLine: "I'm a tech executive with $2M in various accounts, significant RSUs vesting quarterly, and complex compensation. I need more than basic investment management - tax optimization, estate planning coordination, equity compensation strategy. What's your experience with clients at my level of complexity?",
      typicalObjections: [
    ...
    TIME_PRESSURED: {
      ...
      openingLine: "I'm 62 and planning to retire in 12 months. I have about $800k saved but I'm worried it's not enough. I need to figure out Social Security timing, how to draw down my accounts, healthcare before Medicare. This is overwhelming and I'm running out of time. Can you help me create a plan quickly?",
      typicalObjections: [
    ...
    HIGH_BUDGET: {
      ...
      personality: "Sophisticated, values relationship and expertise",
      openingLine: "I recently sold my business for $7M and I need comprehensive wealth management - investment strategy, tax planning, estate planning, charitable giving. I'm interviewing advisors who specialize in serving families at this level. What's your approach to working with clients who've had liquidity events?",
      typicalObjections: [
...
```typescript
{
  ...
  serviceDescription: "I provide expert legal guidance and representation to help clients navigate complex legal matters with confidence and clarity.",
  
  targetClientType: "Individuals, small businesses, startups, families",
  ...
  scenarios: {
    PRICE_SENSITIVE: {
      ...
      openingLine: "Hi, I need help with [estate planning/contract review/small business setup]. I've looked at doing this myself with online tools but I'm worried about making mistakes. My budget is around $2-5k. Can you give me a flat fee quote and help me understand if I really need a lawyer for this?",
      typicalObjections: [
    ...
    INDECISIVE: {
      ...
      openingLine: "I have a situation with [contract dispute/employment issue/property matter]. I'm not sure if I should pursue this legally or try to resolve it another way. I have about $10k I could spend but I don't want to throw money away if my case isn't strong. What's your honest assessment?",
      typicalObjections: [
    ...
    DEMANDING: {
      ...
      openingLine: "I'm dealing with a complex [business dispute/IP issue/regulatory matter]. This is high-stakes for my company. I need a lawyer who is strategic, responsive, and thinks like a business owner, not just applying law textbook answers. What's your experience with cases like this?",
      typicalObjections: [
    ...
    TIME_PRESSURED: {
      ...
      openingLine: "I have a [hearing/deadline/filing] in 2 weeks and I need representation immediately. My previous lawyer [withdrew/wasn't effective/ghosted me]. I know the timing is terrible. Can you take this on and get up to speed quickly? I can pay rush fees if needed.",
      typicalObjections: [
    ...
    HIGH_BUDGET: {
      ...
      personality: "Sophisticated, values expertise and white-glove service",
      openingLine: "I need representation for [major litigation/M&A transaction/estate planning for family business]. This is complex and will require a team with specialized expertise. Budget isn't the primary concern - I need the best outcome. What's your experience leading this type of engagement?",
      typicalObjections: [
...
```typescript
{
  ...
  serviceDescription: "I help businesses grow through data-driven marketing strategies, compelling brand storytelling, and measurable results across digital channels.",
  
  targetClientType: "Small businesses, startups, e-commerce brands, B2B companies",
  ...
  scenarios: {
    PRICE_SENSITIVE: {
      ...
      openingLine: "Hi, I run a small [local service business/e-commerce store] and I've tried Facebook ads and SEO before but didn't see results. I have maybe $2-3k/month budget but I need to see actual customers, not just traffic or impressions. Can you show me specific results you've delivered for businesses like mine?",
      typicalObjections: [
    ...
    INDECISIVE: {
      ...
      openingLine: "I know I need marketing but I'm not sure where to focus. Should I do Google Ads? Social media? SEO? Email? I have about $5k/month budget. Everyone tells me something different. How do you help clients figure out the right marketing mix for their business?",
      typicalObjections: [
    ...
    DEMANDING: {
      ...
      openingLine: "We're a fast-growing [SaaS/e-commerce/B2B] company doing $3M ARR and we need to hit $10M next year. Current marketing isn't scaling. We need a strategic partner who can build a demand gen engine, not just run ads. Budget is $15k+/month for the right team. What's your experience scaling companies like ours?",
      typicalObjections: [
    ...
    TIME_PRESSURED: {
      ...
      personality: "Urgent, decisive, trusts expertise",
      openingLine: "We're launching a new [product/service] in 6 weeks and we need marketing campaigns ready to go on day one. We have $8k/month budget and need everything set up fast - ads, landing pages, email sequences. Can you handle this timeline and deliver quality work under pressure?",
      typicalObjections: [
    ...
    HIGH_BUDGET: {
      ...
      openingLine: "We're a $20M company and we're looking for a full-service marketing partner - strategy, creative, media buying, content, everything. Budget is $25k+/month. We need an agency that can think strategically and execute flawlessly across all channels. What's your approach to becoming an integrated partner vs. just a vendor?",
      typicalObjections: [
...
### **8. Constructors (General Contractors)**
...
```typescript
{
  ...
  serviceDescription: "I deliver quality construction and renovation projects on time and on budget, with clear communication and craftsmanship you can trust.",
  
  targetClientType: "Homeowners, property developers, commercial clients",
  
  typicalBudgetRange: "$20,000 - $500,000+ depending on project scope",
  ...
  scenarios: {
    PRICE_SENSITIVE: {
      ...
      openingLine: "Hi, I'm looking to remodel my kitchen and I have about $40k budget. I've gotten quotes from $35k to $65k for similar scopes. I want quality work but I need to stay on budget. How do you price kitchen remodels and what does your quote actually include?",
      typicalObjections: [
    ...
    INDECISIVE: {
      ...
      openingLine: "We want to add a master suite addition to our home. Budget is around $80k but we're not sure exactly what we want - should we include a walk-in closet? What about the bathroom finishes? We have so many decisions to make and we don't want to regret choices later. How do you help clients figure this out?",
      typicalObjections: [
    ...
    DEMANDING: {
      ...
      openingLine: "I'm building a custom [home/high-end renovation] and I need a contractor who can execute at the highest level. Budget is $250k+. I have a specific vision, I expect daily updates, and I've worked with contractors before who disappointed me. What makes you different and can you handle this level of client?",
      typicalObjections: [
    ...
    TIME_PRESSURED: {
      ...
      openingLine: "I have a rental property that needs a full renovation before my new tenant moves in 8 weeks from now. Budget is $60k. I know the timeline is tight but I'm losing rental income every day it's not done. Can you complete this on time and what would it cost to expedite?",
      typicalObjections: [
...
### **1. Update Registration UI**

```typescript
// app/(auth)/register/page.tsx

const INDUSTRIES = [
  { value: 'mortgage_advisory', label: 'Mortgage Advisory' },
  ...
  { value: 'constructors', label: 'Construction/Contractors' },
];
...
export async function POST(req: NextRequest) {
  const { email, password, name, businessName, industry } = await req.json();
  
  // Create tenant
  const tenant = await prisma.tenant.create({
    data: {
      businessName,
      industry, // ← Store selected industry
      // ... other fields
    }
  });
  
  // Get template for selected industry
  const template = INDUSTRY_TEMPLATES[industry];
  
  // Create BusinessProfile seeded with template
  await prisma.businessProfile.create({
    data: {
      ...
      // Start with 20% completion (has basic template)
      completionPercentage: 20,
      
      // Extracted fields start as null
...
The existing Business Profile page should be updated to show TWO tabs:

```
Business Profile
┌────────────────────────────────────────┐
│ [Basic Info] [Extracted Patterns] ←─ Tabs
├────────────────────────────────────────┤
│                                        │
...
### **Tab 1: Basic Info (User-Editable)**
...
<Tabs defaultValue="basic-info">
  <TabsList>
    ...
    <TabsTrigger value="extracted-patterns">Extracted Patterns</TabsTrigger>
  </TabsList>
        ...
        <form onSubmit={handleSave}>
          ...
          <div className="mb-4">
            ...
            <p className="text-sm text-gray-500 mt-1">
              Change this if you want different simulation scenarios
            </p>
          </div>
          
          {/* Service Description (Textarea - Editable) */}
          <div className="mb-4">
          ...
          {/* Target Client Type (Input - Editable) */}
          <div className="mb-4">
          ...
          {/* Typical Budget Range (Input - Editable) */}
          <div className="mb-4">
          ...
          {/* Common Questions (List - Editable) */}
          <div className="mb-4">
            <Label>Common Client Questions</Label>
            {commonClientQuestions.map((q, i) => (
  ...
  <TabsContent value="extracted-patterns">
    {/* See next section */}
  </TabsContent>
</Tabs>
```

### **Tab 2: Extracted Patterns (Read-Only, Auto-Generated)**
    ...
    <CardContent>
      ...
      <div className="mb-8">
        ...
        <p className="text-sm text-gray-500 mt-2">
          ...
          Complete {getSimulationsNeededForNextLevel()} more to reach{' '}
          {getNextCompletionLevel()}%.
        </p>
      </div>
      ...
      {completionPercentage > 20 ? (
        <>
              ...
              <div className="bg-gray-50 p-4 rounded">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="font-medium">{communicationStyle.tone}</p>
                  </div>
                  <div>
                    <span className="text-sm text-gray-600">Style:</span>
                    <p className="font-medium">{communicationStyle.style}</p>
                  </div>
                </div>
                {communicationStyle.keyPhrases && (
                  <div className="mt-3">
                    <span className="text-sm text-gray-600">Key Phrases:</span>
                    <div className="flex flex-wrap gap-2 mt-1">
                      {communicationStyle.keyPhrases.map((phrase, i) => (
            ...
            <div className="mb-6">
              ...
              <div className="bg-gray-50 p-4 rounded">
                <ul className="space-y-2">
                  {pricingLogic.minBudget && (
                    <li className="flex items-start">
                      <CheckCircle className="h-5 w-5 text-green-500 mr-2 mt-0.5" />
                      <span>Minimum budget: ${pricingLogic.minBudget.toLocaleString()}</span>
                    </li>
                  )}
                  {pricingLogic.maxBudget && (
                    <li className="flex items-start">
                      <CheckCircle className="h-5 w-5 text-green-500 mr-2 mt-0.5" />
                      <span>Maximum budget: ${pricingLogic.maxBudget.toLocaleString()}</span>
                    </li>
                  )}
                  {pricingLogic.flexibilityFactors?.map((factor, i) => (
                    <li key={i} className="flex items-start">
                      <CheckCircle className="h-5 w-5 text-green-500 mr-2 mt-0.5" />
                      <span>Flexible on: {factor}</span>
                    </li>
                  ))}
                </ul>
              </div>
          ...
          {qualificationCriteria && (
            <div className="mb-6">
              <h3 className="font-semibold mb-2">Qualification Criteria</h3>
              <div className="bg-gray-50 p-4 rounded">
                <div className="mb-4">
                  <h4 className="text-sm font-medium text-gray-700 mb-2">Deal Breakers:</h4>
                  <ul className="space-y-1">
                    {qualificationCriteria.dealBreakers?.map((db, i) => (
                      <li key={i} className="flex items-start">
                ...
                <div>
                  ...
                  <ul className="space-y-1">
                    {qualificationCriteria.greenFlags?.map((gf, i) => (
                      <li key={i} className="flex items-start">
                        <CheckCircle className="h-5 w-5 text-green-500 mr-2 mt-0.5" />
                        <span>{gf}</span>
                      </li>
          ...
          {objectionHandling && (
            <div className="mb-6">
              <h3 className="font-semibold mb-2">Objection Handling</h3>
              <div className="bg-gray-50 p-4 rounded space-y-3">
                {Object.entries(objectionHandling).map(([type, response], i) => (
                  <div key={i}>
...
## Simulation Prompt Generation (Using Templates + Patterns)
...
export async function generateSimulationPrompt(
  ...
  // Get business profile (has BOTH template AND extracted data)
  ...
  // ENHANCE with extracted patterns if available
  
  // 1. Use actual service description if customized
  if (profile.serviceDescription && 
      profile.serviceDescription !== template.serviceDescription) {
    // User edited the description, incorporate it
    enhancedPersona.openingLine = enhancedPersona.openingLine.replace(
      /your (services|consulting|work)/i,
      profile.serviceDescription.slice(0, 50) // Use excerpt
    );
  }
  
  // 2. Use actual pricing logic if extracted
  if (profile.pricingLogic) {
    const pricing = profile.pricingLogic as any;
    
    if (pricing.minBudget || pricing.maxBudget) {
      // Adjust budget in persona based on actual pricing
      const budgetRange = `$${pricing.minBudget?.toLocaleString()} - $${pricing.maxBudget?.toLocaleString()}`;
      enhancedPersona.budget = budgetRange;
    }
    
    if (pricing.dealBreakers) {
      // Add actual deal-breakers as potential objections
      enhancedPersona.typicalObjections = [
        ...enhancedPersona.typicalObjections,
        ...pricing.dealBreakers.map(db => `What if ${db}?`)
      ];
    }
  }
  
  // 3. Use actual qualification criteria if extracted
  if (profile.qualificationCriteria) {
    const criteria = profile.qualificationCriteria as any;
    
    if (criteria.dealBreakers) {
      // AI should test these specific deal-breakers
      enhancedPersona.painPoints = [
        ...enhancedPersona.painPoints,
  ...
  // 4. Match communication style if extracted
  if (profile.communicationStyle) {
    const style = profile.communicationStyle as any;
    
    // Adjust personality to match extracted style
    if (style.tone) {
      enhancedPersona.personality += `, responds well to ${style.tone} communication`;
    }
  }
  
  // Build system prompt
  const systemPrompt = `
You are roleplaying as a potential client for a ${template.displayName} professional.

SCENARIO: ${scenarioType}

CLIENT PROFILE:
- Type: ${enhancedPersona.clientType}
- Budget: ${enhancedPersona.budget}
- Personality: ${enhancedPersona.personality}

PAIN POINTS:
${enhancedPersona.painPoints.map(p => `- ${p}`).join('\n')}

YOUR BEHAVIOR:
- Stay in character as this specific client type
- Start with this opening line (or similar): "${enhancedPersona.openingLine}"
- Ask realistic questions and raise objections throughout the conversation
- Use these typical objections when appropriate:
${enhancedPersona.typicalObjections.map(o => `  - ${o}`).join('\n')}
...
Target clients: ${profile.targetClientType || template.targetClientType}
Typical budget range: ${profile.typicalBudgetRange || template.typicalBudgetRange}
...
### **Update Simulation Start Endpoint:**
...
export const POST = withAuth(async (req: AuthenticatedRequest) => {
  const { scenarioType } = await req.json();
  const { tenantId } = req.auth;
  
  // Generate prompt (uses template + extracted patterns)
  const systemPrompt = await generateSimulationPrompt(scenarioType, tenantId);
...
## Adding New Industries (Future-Proof Design)
...
// To add a new industry, simply add to this object:

export const INDUSTRY_TEMPLATES = {
  // ... existing 8 industries ...
  
  // NEW INDUSTRY - Copy this template
  personal_training: {
    industry: "personal_training",
    displayName: "Personal Training",
    description: "Fitness coaching and personal training",
    
    serviceDescription: "I help clients achieve their fitness goals through personalized training programs and nutrition guidance.",
    
    targetClientType: "Individuals looking to lose weight, build muscle, or improve overall fitness",
    
    typicalBudgetRange: "$200 - $600 per month for training packages",
    
    commonClientQuestions: [
      "How often should we train per week?",
      "Do you provide nutrition plans?",
      "What results can I expect?",
      "Do you train at my gym or yours?",
      "What's your training philosophy?",
    ],
    
    scenarios: {
      PRICE_SENSITIVE: {
        clientType: "Budget-conscious fitness beginner",
        budget: "$150-200/month, price shopping",
        painPoints: ["Gym membership already expensive", "Unsure if trainer is worth it"],
        personality: "Hesitant, needs value proof",
        openingLine: "Hi, I'm interested in personal training but I'm already paying for a gym membership. Can you explain what I get for $200/month and why it's worth it?",
        typicalObjections: ["Can I do this on my own with YouTube?", "That seems expensive"]
      },
      // ... other scenarios
    }
  }
...
### **Database:**
1. BusinessProfile table already exists - just needs manual fields added
2. Migration to add industry, serviceDescription, targetClientType, etc.

### **Registration:**
1. Add industry dropdown to register page
2. Update register API to seed BusinessProfile with template

### **Business Profile Page:**
1. Add two tabs: Basic Info (editable) + Extracted Patterns (read-only)
2. Basic Info shows industry, service description, target client, etc.
3. Extracted Patterns shows communication style, pricing, criteria (only after simulations)

### **Simulations:**
1. Update prompt generation to use template + extracted patterns
2. New users get realistic simulations from day 1
3. Experienced users get personalized simulations

### **Template Library:**
1. Create 8 industry templates with 5 scenarios each
2. Easy to add more industries later
3. Templates stored in code (not database for now)

---

## Testing Checklist

- [ ] Registration shows industry dropdown
- [ ] Selecting industry creates BusinessProfile with template data
- [ ] Business Profile page shows Basic Info tab with editable fields
- [ ] Business Profile page shows Extracted Patterns tab (empty for new users)
- [ ] First simulation uses template data (industry-specific AI persona)
- [ ] After simulation, pattern extraction updates extracted fields
- [ ] Completion percentage increases after each simulation
- [ ] Future simulations use BOTH template AND extracted patterns
- [ ] User can edit Basic Info fields and save changes
- [ ] Reset to defaults button restores template values
- [ ] All 8 industries have working templates
- [ ] Adding 9th industry is straightforward
